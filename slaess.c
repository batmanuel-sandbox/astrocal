 ...
 ***************************************************************************
 *
 *  slaess - Initialize SLALIB structure for ESSENCE
 */
void slaess(r,d,jd,w,kk,ss)
double r,d,jd,w;
struct key *kk;
struct sla *ss;
{
...
/*
 * Passed or specific to CTIO site
 */
	ss->epoch = kk->nomepoch;
	ss->ira = r;
	ss->idec = d;
	ss->mjd = jd;
	ss->olon = -(70.0 + 48.9/60.0)/kk->radian;
	ss->olat = -(30.0 + 9.9/60.0)/kk->radian;
	ss->elev = 2215.0;
	ss->temp = 285.0;
